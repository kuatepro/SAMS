...
<body>
  <div class="wrap">
    ...
    <section class="panel">
      <h2>Weeks (Mon → Fri)</h2>
      <div id="weeksContainer" class="weeks"></div>
    </section>
  ...
  const attendanceData = collectAttendanceData(); // use the updated function

  // Send to backend
  const res = await fetch('save_attendance.php', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({attendance: attendanceData, class_id: window.SAMS_CLASS_ID})
  });
        ...
        week.days.forEach(d => {
          if (d) {
            ...
            if (checked) {
              records.push({
                student_id: stu.id,
                name: stu.name, // include student name
